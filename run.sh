...
readonly CONST_QEMU_STORAGE_ARGS="-drive driver=raw,index=0,if=floppy,file=floppy0.img -drive driver=raw,node-name=hdd0,file.driver=file,file.filename=hdd0.img -drive driver=raw,file=nvm.img,if=none,id=nvm -device nvme,serial=deadbeef,drive=nvm"
