...
readonly CONST_QEMU_STORAGE_ARGS="-drive driver=raw,index=0,if=floppy,file=floppy0.img -drive driver=raw,node-name=hdd0,file.driver=file,file.filename=hdd0.img -device nvme-subsys,id=nvme-subsys-0,nqn=subsys0 -device nvme,serial=deadbeef,subsys=nvme-subsys-0 -drive driver=raw,file=nvme0.img,if=none,id=nvme0 -device nvme-ns,drive=nvme0"
